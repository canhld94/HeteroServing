 ...
 * They are not essential layer but convient when dealing with output of inference engine.
 ***************************************************************************************/
...
namespace ie {
    template <class T>
    static bool sort_score_pair_descend(const std::pair<float,T> p0, const std::pair<float,T> p1) {
    ...
    public:
        explicit detection_output_inference(const CNNLayer *layer): cnn_layer(*layer) {
            try {
                if (cnn_layer.insData.size() != 3) {
                    THROW_IE_EXCEPTION << "Incorrect number of input edges for layer " << cnn_layer.name;
                }
                if (cnn_layer.outData.empty()) {
                    THROW_IE_EXCEPTION << "Incorrect number of output edges for layer " << cnn_layer.name;
                }
                ...
                _variance_encoded_in_target = cnn_layer.GetParamAsBool("variance_encoded_in_target",false);
                ...
                std::string code_type_str = cnn_layer.GetParamAsString("code_type", "cafe.PriorBoxParameter.CORNER");
                _code_type = (code_type_str == "cafe.PriorBoxParameter.CENTER_SIZE" ? code_type::CENTER_SIZE : code_type::CORNER);

                auto priors_ptr = cnn_layer.insData[idx_priors].lock();
                if (!priors_ptr) {
                    THROW_IE_EXCEPTION << "Pointer to the input data of Layer" << cnn_layer.name << " is expired";
                }
                ...
                unsigned int conf_size = conf_dims[conf_dims.size() - 2]* conf_dims[conf_dims.size() - 1];
                ...
                SizeVector bboxes_size{static_cast<size_t>(_num),static_cast<size_t>(_num_classes),static_cast<size_t>(_num_priors)};
                _decoded_bboxes = make_shared_blob<int>({Precision::FP32,  bboxes_size, NCHW});
                _decoded_bboxes->allocate();
                
                SizeVector buf_size{static_cast<size_t>(_num),static_cast<size_t>(_num_classes),static_cast<size_t>(_num_priors)};
                _buffer = make_shared_blob<int>({Precision::I32,  buf_size, {buf_size, {0,1,2}}});
                _buffer->allocate();
                
                SizeVector indices_size{static_cast<size_t>(_num),static_cast<size_t>(_num_classes),static_cast<size_t>(_num_priors)};
                _indices = make_shared_blob<int>({Precision::I32,  indices_size, {indices_size, {0,1,2}}});
                _indices->allocate();
                
                SizeVector detection_size{static_cast<size_t>(_num*_num_classes),static_cast<size_t>(_num_classes),static_cast<size_t>(_num_priors)};
                _detections_count = make_shared_blob<int>({Precision::I32, detection_size, C});
                _indices->allocate();
                ...
                SizeVector decoded_bboxes_size{static_cast<size_t>(_num*_num_classes),static_cast<size_t>(_num_classes),static_cast<size_t>(_num_priors)};
                _bbox_sizes = make_shared_blob<int>({Precision::I32, decoded_bboxes_size, {decoded_bboxes_size, {0,1,2}}});
                ...
                throw std::logic_error(std::string("Can't create detection output from given CNN layer: ") + e.what());
            }
        };
        ...
        StatusCode execute(std::vector<Blob::Ptr>& inputs, std::vector<Blob::Ptr>& outputs, ResponseDesc *resp) noexcept {
            ...
            for (int n = 0; n < N; ++n) {
                ...
                for (int c = 0; c < _num_classes; ++c) {
                    ...
                    const float *pconf = reordered_conf_data + n*_num_classes + c*_num_priors;
                    const float *pboxes;
                    const float *psizes;
                    if (_share_location) {
                ...
                if (_keep_top_k > -1 && detections_total > _keep_top_k) {
                    ...
                    for (int c = 0; c < _num_classes; ++c) {
                        ...
                        int *pindices = indices_data + n*_num_classes*_num_priors + c*_num_priors;
                        float *pconf = reordered_conf_data + n*_num_classes + c*_num_priors;

...
class listener : public std::enable_shared_from_this<listener> {
 ...
 public:
  ...
  listener(net::io_context& _ioc, tcp::endpoint endpoint,
           connection_mq::ptr& _connq)
      : ioc(_ioc), acceptor(net::make_strand(_ioc)), connq(_connq) {
    beast::error_code ec;
    // open the acceptor
 ...
 *
 */
class async_listening_worker
    : public std::enable_shared_from_this<async_listening_worker> {
 private:
  connection_mq::ptr connq;
  std::string address;
  std::string port;
...
class async_http_worker
    : public std::enable_shared_from_this<async_http_worker> {
 private:
  connection_mq::ptr connq;
 ...
 public:
  ...
  async_http_worker(connection_mq::ptr& _connq, http_mq::ptr& _cpu_taskq,
      ...
      : connq(_connq),
        cpu_taskq(_cpu_taskq),
        fpga_taskq(_fpga_taskq),
  ...
  void operator()() {
    try {
      for (;;) {
        auto sock = connq->pop();
        // read from sock
        PROFILE_DEBUG("Read From Socket", beast::flat_buffer buffer;
                      beast_basic_request req; beast::error_code ec;
                      bool close = false;
 ...
 private:
  ...
  void request_handler(beast_basic_request&& req, tcp::socket&& sock) {
    ...
    if (req.method() != http::verb::get && req.method() != http::verb::head &&
      ...
      return sender(
          error_message(req, http::status::bad_request, "Unknown HTTP-method"));
    // Request path must be absolute and not contain "..".
    std::string target = request_resolve(req.target(), ec);
    if (target.size() == 0) {
      return sender(error_message(req, http::status::bad_request,
    ...
    // Respond to HEAD request, alway just send the basic information of the
    // server
    if (req.method() == http::verb::head) {
      ...
      if (target == "inference/cpu") {
        if (cpu_taskq) {
          cpu_taskq->push(
              std::make_shared<http_conn>(std::move(sock), std::move(req)));
        } else {
          sender(error_message(req, http::status::not_implemented,
        ...
        sender(error_message(req, http::status::bad_request,
                             "Illegal HTTP method"));
      }
    }
  }
  std::string request_resolve(beast::string_view const& target,
    ...
    static const std::set<std::string> resources = {
        ...
        "v1"
        "metadata",
        "inference/cpu", "inference/fpga", "inference/gpu"};
  ...
  std::string greeting() {
    ...
    res.put<std::string>("message",
                         "welcome to NCL inference server version 1");
    JSON what_next;
...
class async_inference_worker {
 ...
 public:
  ...
  async_inference_worker(inference_engine::ptr& _Ie, http_mq::ptr& _taskq,
                         response_mq::ptr& _resq)
  ...
  void operator()() {
    try {
      ...
      for (;;) {
        ...
        // push to resq
        resq->push(std::make_shared<inference_output>(
            std::move(sock), std::move(detection_out)));
      }
    } catch (const std::exception& e) {
      std::cerr << e.what() << std::endl;
    }
  ...
  void operator()() {
    try {
      ...
      for (;;) {
        ...
        auto body = ss.str();
        auto const size = body.size();
        beast_basic_response res{std::piecewise_construct,

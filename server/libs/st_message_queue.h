...
template <class Key, class LiteralKey, LiteralKey reset_state,
          class CondVar = std::condition_variable, class Mutex = std::mutex,
...
class simple_bell {
 ...
 public:
  ...
  /**
   * @brief Ring the bell
   * @details This function should be call by consumer to wake up the owner of
   * the bell
   * @param set_state
   */
...
class message {
 ...
 public:
   ...
   * @param _bell
   */
  message(DataPtr& _data, Ssize& _size, ResponsePtr _predictions,
          BellPtr& _bell)

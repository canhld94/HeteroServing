 ...
 * @b About: This file implement some extra layer that OpenVino doesn't support.
 * They are not essential layer but convient when dealing with output of
 *inference engine.
 ***************************************************************************************/
...
namespace ie {
template <class T>
static bool sort_score_pair_descend(const std::pair<float, T> p0,
                                    const std::pair<float, T> p1) {
  return p0.first > p1.first;
}

struct ConfidenceComparator {
  explicit ConfidenceComparator(const float* conf_data)
      : _conf_data(conf_data) {}
...
static inline float JaccardOverlap(const float* decoded_bbox,
                                   const float* bbox_sizes, const int idx1,
                                   const int idx2) {
...
/**
 * @brief detection parsing layer for RCNN family
 *
 */
class detection_output_inference {
 private:
  CNNLayer cnn_layer;

  const int idx_location = 0;
  const int idx_confidence = 0;
  ...
  void decodeBBoxes(const float* prior_data, const float* loc_data,
                    const float* variance_data, float* decoded_bboxes,
                    float* decoded_bbox_sizes, int* num_priors_actual, int n) {
    ...
    for (int p = 0; p < num_priors_actual[n]; ++p) {
      ...
      if (_code_type == code_type::CORNER) {
      ...
      } else if (_code_type == code_type::CENTER_SIZE) {
        ...
        if (_variance_encoded_in_target) {
          ...
          decode_bbox_center_x =
              variance_data[p * 4 + 0] * loc_xmin * prior_width +
              prior_center_x;
          decode_bbox_center_y =
              variance_data[p * 4 + 1] * loc_ymin * prior_height +
              prior_center_y;
          decode_bbox_width =
              std::exp(variance_data[p * 4 + 2] * loc_xmax) * prior_width;
          decode_bbox_height =
              std::exp(variance_data[p * 4 + 3] * loc_ymax) * prior_height;
        }
  ...
  void nms(const float* conf_data, const float* bboxes, const float* sizes,
    ...
    int num_output_scores =
        (_top_k == -1 ? count : std::min<int>(_top_k, count));

    std::partial_sort_copy(indices, indices + count, buffer,
 ...
 public:
  explicit detection_output_inference(const CNNLayer* layer)
      : cnn_layer(*layer) {
    try {
      if (cnn_layer.insData.size() != 3) {
        THROW_IE_EXCEPTION << "Incorrect number of input edges for layer "
                           << cnn_layer.name;
      }
      if (cnn_layer.outData.empty()) {
        THROW_IE_EXCEPTION << "Incorrect number of output edges for layer "
                           << cnn_layer.name;
      }

      _num_classes = cnn_layer.GetParamAsInt("num_classes");
      _background_label_id = cnn_layer.GetParamAsInt("background_label_id", 0);
      _top_k = cnn_layer.GetParamAsInt("top_k", -1);
      _variance_encoded_in_target =
          cnn_layer.GetParamAsBool("variance_encoded_in_target", false);
      _keep_top_k = cnn_layer.GetParamAsInt("keep_top_k", -1);
      _nms_threshold = cnn_layer.GetParamAsFloat("nms_threshold");
      _share_location = cnn_layer.GetParamAsBool("share_location", true);
      _normalized = cnn_layer.GetParamAsBool("normalized", true);
      _image_width = cnn_layer.GetParamAsInt("image_width", 1);
      _image_height = cnn_layer.GetParamAsInt("image_height", 1);
      _prior_size = _normalized ? 4 : 5;
      _offset = _normalized ? 0 : 1;
      _num_loc_classes = _share_location ? 1 : _num_classes;

      std::string code_type_str = cnn_layer.GetParamAsString(
          "code_type", "cafe.PriorBoxParameter.CORNER");
      _code_type = (code_type_str == "cafe.PriorBoxParameter.CENTER_SIZE"
                        ? code_type::CENTER_SIZE
                        : code_type::CORNER);

      auto priors_ptr = cnn_layer.insData[idx_priors].lock();
      if (!priors_ptr) {
        THROW_IE_EXCEPTION << "Pointer to the input data of Layer"
                           << cnn_layer.name << " is expired";
      }
      auto prior_dims = priors_ptr->dims;
      int priors_size = prior_dims[0] * prior_dims[1];

      auto loc_ptr = cnn_layer.insData[idx_location].lock();
      if (!loc_ptr) {
        THROW_IE_EXCEPTION << "Pointer to the input data of Layer"
                           << cnn_layer.name << " is expired";
      }
      ...
      int loc_size =
          loc_dims[loc_dims.size() - 2] * loc_dims[loc_dims.size() - 1];

      auto conf_ptr = cnn_layer.insData[idx_confidence].lock();
      if (!conf_ptr) {
        THROW_IE_EXCEPTION << "Pointer to the input data of Layer"
                           << cnn_layer.name << " is expired";
      }
      ...
      unsigned int conf_size =
          conf_dims[conf_dims.size() - 2] * conf_dims[conf_dims.size() - 1];
      ...
      _decoded_bboxes =
          make_shared_blob<int>({Precision::FP32, bboxes_size, NCHW});
      _decoded_bboxes->allocate();

      SizeVector buf_size{static_cast<size_t>(_num),
                          static_cast<size_t>(_num_classes),
                          static_cast<size_t>(_num_priors)};
      _buffer = make_shared_blob<int>(
          {Precision::I32, buf_size, {buf_size, {0, 1, 2}}});
      _buffer->allocate();
      ...
      SizeVector detection_size{static_cast<size_t>(_num * _num_classes),
                                static_cast<size_t>(_num_classes),
                                static_cast<size_t>(_num_priors)};
      _detections_count =
          make_shared_blob<int>({Precision::I32, detection_size, C});
      _indices->allocate();

      SizeVector conf_size1 = {conf_size, 1};
      _reordered_conf =
          make_shared_blob<float>({Precision::FP32, conf_size1, ANY});
      _reordered_conf->allocate();

      SizeVector decoded_bboxes_size{static_cast<size_t>(_num * _num_classes),
                                     static_cast<size_t>(_num_classes),
                                     static_cast<size_t>(_num_priors)};
      _bbox_sizes = make_shared_blob<int>({Precision::I32,
                                           decoded_bboxes_size,
                                           {decoded_bboxes_size, {0, 1, 2}}});
      _bbox_sizes->allocate();

      SizeVector num_priors_actual_size{static_cast<size_t>(_num)};
      _num_priors_actual =
          make_shared_blob<int>({Precision::I32, num_priors_actual_size, C});
      _num_priors_actual->allocate();
    } catch (const InferenceEngineException& e) {
      throw std::logic_error(
          std::string("Can't create detection output from given CNN layer: ") +
          e.what());
    }
  };
  ...
  StatusCode execute(std::vector<Blob::Ptr>& inputs,
    ...
    for (int n = 0; n < N; ++n) {
      if (_share_location) {
        ...
        decodeBBoxes(ppriors, ploc, prior_variances, pboxes, psizes,
                     num_priors_actual, n);
      } else {
        for (int c = 0; c < _num_loc_classes; ++c) {
          ...
          const float* ploc =
              loc_data + n * 4 * _num_loc_classes * _num_priors + c * 4;
          float* pboxes = decoded_bboxes_data +
                          n * 4 * _num_loc_classes * _num_priors +
                          c * 4 * _num_priors;
          float* psizes = bbox_sizes_data + n * _num_priors;
          decodeBBoxes(ppriors, ploc, prior_variances, pboxes, psizes,
                       num_priors_actual, n);
    ...
    for (int n = 0; n < N; ++n) {
      for (int c = 0; c < _num_classes; ++c) {
        for (int p = 0; p < _num_priors; ++p) {
          reordered_conf_data[n * _num_priors * _num_classes + c * _num_priors +
                              p] =
              conf_data[n * _num_priors * _num_classes + p * _num_classes + c];
        }
      }
    }
    ...
    for (int n = 0; n < N; ++n) {
      ...
      for (int c = 0; c < _num_classes; ++c) {
        ...
        int* pindices =
            indices_data + n * _num_classes * _num_priors + c * _num_priors;
        int* pbuffer = buffer_data + c * _num_priors;
        int* pdetections = detections_data + n * _num_classes + c;

        const float* pconf =
            reordered_conf_data + n * _num_classes + c * _num_priors;
        const float* pboxes;
        const float* psizes;
        if (_share_location) {
          ...
          pboxes = decoded_bboxes_data + n * 4 * _num_classes * _num_priors +
                   c * 4 * _num_priors;
          psizes = bbox_sizes_data + n * _num_classes * _num_priors +
                   c * _num_priors;
        }

        nms(pconf, pboxes, psizes, pbuffer, pindices, *pdetections,
            num_priors_actual[n]);
      }
      ...
      if (_keep_top_k > -1 && detections_total > _keep_top_k) {
        ...
        for (int c = 0; c < _num_classes; ++c) {
          ...
          for (int i = 0; i < detections; ++i) {
            int idx = pindices[i];
            conf_index_class_map.push_back({pconf[idx], {c, idx}});
          }
        }
        ...
        //
        memset(detections_data + n * _num_classes, 0,
               _num_classes * sizeof(int));
        ...
        for (size_t j = 0; j < conf_index_class_map.size(); ++j) {
          ...
          int* pindices = indices_data =
              n * _num_classes * _num_priors + label * _num_priors;
    ...
    for (int n = 0; n < N; ++n) {
      ...
      const float* pboxes =
          decoded_bboxes_data + n * _num_priors * 4 * _num_loc_classes;
      ...
      for (int c = 0; c < _num_classes; ++c) {
        for (int i = 0; i < detections_data[n * _num_classes + c]; ++i) {
          ...
          float xmin = _share_location
                           ? pboxes[idx * 4 + 0]
                           : pboxes[c * 4 * _num_priors + idx * 4 + 0];
          float ymin = _share_location
                           ? pboxes[idx * 4 + 1]
                           : pboxes[c * 4 * _num_priors + idx * 4 + 1];
          float xmax = _share_location
                           ? pboxes[idx * 4 + 2]
                           : pboxes[c * 4 * _num_priors + idx * 4 + 2];
          float ymax = _share_location
                           ? pboxes[idx * 4 + 3]
